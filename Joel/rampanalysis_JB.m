function  [blue_ramp, red_ramp ODI Ratio_AMPA_NMDA]=rampanalysis(list, idx, voltage, pathName, fc, show, ramp_rtrace, user, filterephys,adata_dir,injection_order);
%SW181229
...
%AMPA
blue_ramp_70=blue_ramp.neg_peak2(1,:);
blue_laser_70=blue_ramp.neg_laser_amp(1,:);
red_ramp_70=red_ramp.neg_peak1(2,:);
red_laser_70=red_ramp.neg_laser_amp(2,:);
blue_constant_70=blue_ramp.neg_peak2(2,:);
blue_claser_70=blue_ramp.neg_laser_amp(2,:);
%NMDA
if ~isempty(blue_ramp.pos_peak2)
    blue_ramp_40=blue_ramp.pos_peak2(1,:);
    blue_laser_40=blue_ramp.pos_laser_amp(1,:);
    red_ramp_40=red_ramp.pos_peak1(2,:);
    red_laser_40=red_ramp.pos_laser_amp(2,:);
    blue_constant_40=blue_ramp.pos_peak2(2,:);
    blue_claser_40=blue_ramp.pos_laser_amp(2,:);
else
    blue_ramp_40=[];
    blue_laser_40=[];
    red_ramp_40=[];
...
tresh_nr=4;
%%%%%%%BLUE ONLY AMPA
idx=find(blue_ramp.neg_fail2(1,:)>0)';
d=blue_ramp.neg_peak2(1,:);
%%%%%%%
ze=zeros(11,1);
if length(idx)>=tresh_nr;
    ze(idx)=d(idx);
    b_r_70=ze ;
else
    b_r_70=ze;
...
ze=[];
%%%%%%%RED AMPA
idx=find(red_ramp.neg_fail1(2,:)>0)';
d=red_ramp.neg_peak1(2,:);
%%%%%%%
ze=zeros(11,1);
if length(idx)>=tresh_nr;
    ze(idx)=d(idx);
...
ze=[];
%%%%%%%BLUE CONSTANT AMPA
curr=blue_ramp.neg_fail2(2,:);
curr(1:3)=0;
idx=find(curr>0)';
d=blue_ramp.neg_peak2(2,:);
%%%%%%%
ze=zeros(11,1);
if length(idx)>=tresh_nr;
    ze(idx)=d(idx);
...
%%%%%%%BLUE ONLY NMDA
if ~isempty(blue_ramp.pos_peak2)
    idx=find(blue_ramp.pos_fail2(1,:)>0)';
    d=blue_ramp.pos_peak2(1,:);
    %%%%%%%
    ze=zeros(11,1);
    if length(idx)>=tresh_nr;
        ze(idx)=d(idx);
    ...
    ze=[];
    %%%%%%%RED NMDA
    idx=find(red_ramp.pos_fail1(2,:)>0)';
    d=red_ramp.pos_peak1(2,:);
    %%%%%%%
    ze=zeros(11,1);
    if length(idx)>=tresh_nr;
        ze(idx)=d(idx);
    ...
    ze=[];
    %%%%%%%BLUE CONSTANT NMDA
    curr2=blue_ramp.pos_fail2(2,:);
    curr2(1:3)=0;
    idx=find(curr2>0)';
    d=blue_ramp.pos_peak2(2,:);
    %%%%%%%
    ze=zeros(11,1);
    if length(idx)>=tresh_nr;
        ze(idx)=d(idx);
...
if injection_order==1 %injection order is 1
    %AMPA
    ODI_AMPA=(abs(r_r_70)-abs(b_c_70))./(abs(r_r_70)+abs(b_c_70));
    %NMDA
    ODI_NMDA=(abs(r_r_40)-abs(b_c_40))./(abs(r_r_40)+abs(b_c_40));
else %injection order is 0
    %AMPA
    ODI_AMPA=(abs(b_c_70)-abs(r_r_70))./(abs(r_r_70)+abs(b_c_70));
    %NMDA
    ODI_NMDA=(abs(b_c_40)-abs(r_r_40))./(abs(r_r_40)+abs(b_c_40));
end
%AMPA/NMDA Ratio
if ~isempty(blue_ramp.pos_peak2)
    R_AMPA_NMDA_red=abs(r_r_70)./abs(r_r_40);
    R_AMPA_NMDA_blue=abs(b_c_70)./abs(b_c_40);
else
    R_AMPA_NMDA_red=[];
    R_AMPA_NMDA_blue=[];
end
...
ODI_A_sem=nanstd(ODI_AMPA(5:end,:))/sqrt(sum(~isnan(ODI_AMPA)));
ODI_N=nanmean(ODI_NMDA(5:end,:));
ODI_N_sem=nanstd(ODI_NMDA(5:end,:))/sqrt(sum(~isnan(ODI_NMDA)));
%RATIO AMPA/NMDA average
R_r=nanmean(R_AMPA_NMDA_red(5:end,:));
R_r_sem=nanstd(R_AMPA_NMDA_red(5:end,:))/sqrt(sum(~isnan(R_AMPA_NMDA_red)));
R_b=nanmean(R_AMPA_NMDA_blue(5:end,:));
R_b_sem=nanstd(R_AMPA_NMDA_blue(5:end,:))/sqrt(sum(~isnan(R_AMPA_NMDA_blue)));



ODI.ODI_AMPA=ODI_A;
ODI.ODI_NMDA=ODI_N;
Ratio_AMPA_NMDA.R_r=R_r;
Ratio_AMPA_NMDA.R_b=R_b;
