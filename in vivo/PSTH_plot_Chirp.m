function        [DFpeak F0 F02 DFmean_trials SigF0 delta_max delta_mean delta_integ delta_meanF0 delta_sigmaF0 mean_plotdata plot_data_all] =  PSTH_plot_Chirp(roi_data, ana_data, auxdata, ids, ROI, ratio, single_roi, plotdata, npfct, eyes,  runexclude, varargin)
...
for rec = 1:recs;
    ...
    SamplingFreq1 = regexp(roi_data{rec}.info.ImageDescription, '(?<=scanFrameRate = )\d+\.?\d*', 'match');
    ...
    for ey = 1:eyes;
        ...
        if ~episodic_F0
            for rep = 1:reps
                baseperiodstim = ids.StimBounds{ey}(rep,1)-1-round(F0per*SamplingFreq1):ids.StimBounds{ey}(rep,1)-1;
                if min(baseperiodstim) <1
                    ...
                    F0 = nanmedian(F0_new(:));
                    disp('Using median of prestim F0 distribution as global F0');
                end
                if useF0mode
                    disp('Using mode of prestim F0 distributions as global F0');
                    F0 = mode(F0_new(:));
                end
            end
        end
        if useF0mininterp
            disp('Using interpolation of F0 F0minima for episodic F0 - CHECK! Useful if there is lots of activity in baseline')
            [F0_min_new, ~,F0_min_interp,~] = getF0frombaseline_chirp(raw_data, ids, F0per, SamplingFreq1);
        end
        if noneg;
        ...
        for rep = 1:reps;
            ...
            if episodic_F0 && ~useF0mininterp
                disp('Using episodic F0 per stimulus ')
                baseperiodstim = ids.StimBounds{ey}(rep,1)-1-round(F0per*SamplingFreq1):ids.StimBounds{ey}(rep,1)-1;
                F0 = nanmedian(raw_data(baseperiodstim)); %TR14 -1! before the first stimframe was included - that's of course wrong!
            elseif episodic_F0 && useF0mininterp
                F0 =  F0_min_new(rep,ey);
            end
            
            plotarray_disp =  ids.StimBounds{ey}(rep,1)-1-round(prestim*SamplingFreq1):ids.StimBounds{ey}(rep,2)+floor(SamplingFreq1*poststim);
            auxarray_disp  = auxdata(8,ids.frame_times_level(plotarray_disp(1)):ids.frame_times_level(plotarray_disp(end)));
            auxarray_disp_timebase = (1:length(auxarray_disp)) /  ids.aux_samplingrate;
            ...
            DFpeak(ey,rep)         = nanmax(plot_data(rep,response));
            DFmean_trials(ey,rep)  = nanmean(plot_data(rep,response)); % for ANOVA responder crit
            F02(ey,rep)            = nanmean(plot_data(rep,baseline));
            SigF0(ey,rep)          = nanstd(plot_data(rep,baseline));
            Fend(ey,rep)           = nanmean(plot_data(rep,end-endline:end));
            Fstart(ey,rep)         = nanmean(plot_data(rep,1:startline));
            
            
            if plotdata
                hdl(ey) = subplot(recs,eyes,k);
                
                ptr = plot(timebase(1:downsamp:end),plot_data(rep,1:downsamp:end), 'k', 'LineWidth', .5);
                %                             try
        ...
        if plotdata
            %             hdl(2) = subplot(recs,1,k);
            plot(timebase(1:downsamp:end), mean_plotdata{1}(1:downsamp:end), 'r', 'LineWidth', 1);
            k = k+1;
            ...
            if ey ==1 ;
                ...
                plot(auxarray_disp_timebase, auxarray_disp * abs(yl(1))/10 + yl(1), 'k', 'LineWidth', 1) ;
            elseif ey == 2 ;
