function [DFpeak F02  DFmean_trials SigF0 delta_max delta_mean delta_integ delta_meanF0 delta_sigmaF0 mean_plotdata plot_data_all delta_meanrate smoothrate b a nerds int] = PSTH_plot_chronic_simon(roi_data, ana_data, ids, ROI, ratio, single_roi, plotdata, npfct, eyes, basel, runexclude, varargin)
    ...
    for ey = 1:eyes;
        ...
        if ~axondf_already
            if ~episodic_F0
                ...
                if ~smc_oopsi && detrend
                    ...
                    disp('detrending ratio')
                end
                disp('Using median of F0 distributions as global F0');
                for ori = 1:oris;
            ...
            if useF0mininterp
                ...
                disp('using interpolated prestim period as episodic F0')
            end
            if noneg;
        ...
        for ori = 1:oris;
            ...
            for rep = 1:reps
                ...
                DFmean_trials(ori,rep,ey) = nanmean(plot_data(rep,response)); % for ANOVA responder crit
                ...
                if plotdata
                    hdl(ey,ori) = subplot(recs,oris*eyes + furtherfig,k);
                    if ~smc_oopsi
