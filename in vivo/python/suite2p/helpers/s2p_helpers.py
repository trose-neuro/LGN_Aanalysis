...
Provides a range of batch  functions to s2p through our Munich data storage
...
def batch_s2p_ROI_convert( runops ):
    "Batch run for converting S2P output to ML ROI structure."
    ...
    main_root = runops.get('main_root')
    adata = runops.get('adata')
    save_template = runops.get('save_template')

    # parse main directory for analyzed s2p data
    ...
    #adata_roipath = sorted(adata_roipath)

    #print('files: ' + files[0])
    print('adata_s2ppath: ' + adata_s2ppath[0])
    print('adata_roipath: ' + adata_roipath[0])
    print('exp_work: ' + exp_work[0])

    for ids, val in enumerate(exp_work):
        # load trace data
        F = np.load(os.path.join(adata_s2ppath[ids],'F.npy'))
        Fneu = np.load(os.path.join(adata_s2ppath[ids],'Fneu.npy'))
        spks = np.load(os.path.join(adata_s2ppath[ids],'spks.npy'))
        stat = np.load(os.path.join(adata_s2ppath[ids],'stat.npy'), allow_pickle=True)
        ops = np.load(os.path.join(adata_s2ppath[ids],'ops.npy'), allow_pickle=True).item()
        iscell = np.load(os.path.join(adata_s2ppath[ids],'iscell.npy'))

        print('- - - - - - - -')
        print('Processing exp' + val)
        print('- - - - - - - -')

        ### reconvolve spike traces with exp kernel to get "smoothened" F - Fneu * 0.7
        #ops['tau'] = 0.6
        Freconv =[];
        ...
        for n in range(ncells):
            ...
            Freconv_tmp = convolve(spks[n,:], efilt)
            Freconv.append(Freconv_tmp[:spks.shape[1]])

        # populate ROI array for export
        ncells = len(stat)
        ROI = []
        adataROI = []
        for n in tqdm(range(ncells)):
            cellIDMap = np.zeros((ops['Ly'], ops['Lx']), 'bool')
            ypix = stat[n]['ypix'][~stat[n]['overlap']]
            xpix = stat[n]['xpix'][~stat[n]['overlap']]
            cellIDMap[ypix,xpix] = True
            perimeter = np.asarray(np.where(bwperim(cellIDMap))).T #convert tuple to np array and transpose
            body = np.asarray(np.where(cellIDMap)).T  #convert tuple to np array and transpose
            size = len(body)
            ROI.append({
                'nr': n,
                'group': 1,
                'type': 8,
                'typename': 'bouton',
                'x_CM': ndimage.measurements.center_of_mass(cellIDMap)[0],
                'y_CM': ndimage.measurements.center_of_mass(cellIDMap)[1],
                'x': stat[n]['med'][0],
                'y': stat[n]['med'][1],
                'size': size,
                'perimeter': perimeter,
            ...
            adataROI.append({
                ...
                'activity_F': F[n],
                'activity_neu': Fneu[n],
                'activity_spks': spks[n],
                'activity_reconv': Freconv[n],
                'exp': exp_work[ids]
            })

        #### Export to Matlab - Be aware: needs `cell2mat` call in matlab!

        #make template directory name
        froot = os.path.join(adata_roipath[ids], 'Pixelmaps','exp'+ val,'templates', 'ROIs')
        ftiffroot = os.path.join(adata_roipath[ids], 'Pixelmaps','exp'+ val,'templates')
        ftiffmean = Path(adata_s2ppath[ids]).parts[-6] + '_' + Path(adata_s2ppath[ids]).parts[-4] + '_exp' + val + '_s2p_avg_green.tif'
        ftiffcorr = Path(adata_s2ppath[ids]).parts[-6] + '_' + Path(adata_s2ppath[ids]).parts[-4] + '_exp' + val + '_s2p_corr_green.tif'
        adataname = os.path.join(adata, Path(adata_s2ppath[ids]).parts[-6] , Path(adata_s2ppath[ids]).parts[-4], Path(adata_s2ppath[ids]).parts[-6] + '-' + 's2p_Adata' + '-' + val + '.mat')

        Path(froot).mkdir(parents=True, exist_ok=True)
        print('Made directory (froot) ' + froot)

        savemat(os.path.join(froot, 'ROIs2p.mat'), {'ROI': ROI}, appendmat=True)
        print('Saved ML ROI file ' + os.path.join(froot, 'ROIs2p.mat'))

        savemat(adataname, {'ROI': adataROI}, appendmat=True)
        print('Saved ML adata file ' + adataname)

        if save_template:
            imsave(os.path.join(ftiffroot, ftiffmean), ops['meanImg'] )
            imsave(os.path.join(ftiffroot, ftiffcorr), ops['Vcorr'] )
            print('Saved template tiff ' + os.path.join(ftiffroot, ftiffmean))
            print('Saved template tiff ' + os.path.join(ftiffroot, ftiffcorr))
    print('- - - - - - - -')
    print('Done')
